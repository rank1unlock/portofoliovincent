  ...
  <section id="video-introduction" ref="sectionTarget" class="bg-animated-dark py-20 px-4 relative overflow-hidden">
    ...
    <div class="background-wrapper">
      ...
      <div class="floating-particles">
        <div v-for="i in 20" :key="`particle-${i}`" class="particle" :style="getVideoParticleStyle(i)"></div>
      </div>
    </div>
    ...
    <div class="floating-cyber-elements">
      <div class="cyber-element cyber-element-1" :style="{ transform: `translate3d(${mouseX * -0.02}px, ${mouseY * -0.02}px, 0)` }">
        <div class="cyber-spin-slow">
          <Icon name="mdi:play-circle-outline" class="w-16 h-16 sm:w-24 sm:h-24 text-purple-500/10" />
        </div>
      </div>
    ...
    <div class="floating-cyber-elements">
      <div class="cyber-element cyber-element-1">
        <Icon name="mdi:play-circle-outline" class="w-16 h-16 text-purple-500/10" />
      </div>
      <div class="cyber-element cyber-element-2">
        <Icon name="mdi:movie-open-outline" class="w-20 h-20 text-blue-500/10" />
      </div>
      <div class="cyber-element cyber-element-3">
        <Icon name="mdi:video-vintage" class="w-14 h-14 text-emerald-500/10" />
      </div>
    </div>
        ...
        <div ref="videoTarget" class="transition-all duration-300 ease-out relative">
          ...
          >
            ...
            <div class="video-container">
              ...
              <div v-else class="w-full aspect-video bg-slate-900 flex flex-col justify-center items-center text-slate-500 rounded-lg border border-slate-700">
...
// === 1. STATE UNTUK DATA VIDEO DINAMIS ===
...
onMounted(() => {
  ...
  // Panggil data video dari database saat web dibuka
  fetchVideoData();
...
.cyber-element-1 { top: 20%; left: 15%; animation-delay: 0s; }
.cyber-element-2 { bottom: 20%; right: 10%; animation-delay: -5s; }
.cyber-element-3 { top: 60%; left: 30%; animation-delay: -10s; }

@keyframes float-cyber-element {
  0%, 100% { transform: translateY(0) rotate(0deg) scale(1); }
  33% { transform: translateY(-30px) rotate(45deg) scale(1.1); }
...
@media (prefers-color-scheme: light) {
  .bg-animated-dark {
   background: linear-gradient(-45deg, #000000, #0a0a0a, #121212, #050505, #080808);
  }
...
/* ===================================
   ...
   =================================== */
/* Cahaya Spotlight Mengikuti Mouse */
.interactive-spotlight {
  ...
  z-index: 1;
  will-change: transform;
  /* Transisi dihapus agar benar-benar realtime mengikuti kursor */
}

/* Animasi Parallax Elemen Background */
.cyber-element {
  position: absolute;
  will-change: transform;
  /* Membuat pergerakan mouse menjadi sangat mulus (easing) */
  transition: transform 0.4s cubic-bezier(0.23, 1, 0.32, 1);
}

.cyber-element-1 { top: 15%; left: 10%; }
.cyber-element-2 { bottom: 20%; right: 10%; }
.cyber-element-3 { top: 60%; left: 25%; }

/* Animasi Rotasi Halus di dalam Elemen Parallax */
...
/* Memperkuat Efek 3D pada Video Player */
.enhanced-video-container {
  /* Pastikan transisi untuk transform (3D tilt) ada */
