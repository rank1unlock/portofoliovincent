    ...
    <div class="container">
      <div class="gallery-header" ref="headerRef">
        <div class="header-badge">
          ...
          <span
            >{{ filteredCertificates.length }}+ {{ translations.gallery.badge[currentLang] }}</span
          >
        </div>
      ...
      <div v-show="viewMode === 'carousel'" class="carousel-container" ref="carouselRef">
        ...
        <button
          ...
          :disabled="activeIndex === filteredCertificates.length - 1"
        >
            ...
            >
              <div
                :class="[
                  ...
                  { 'is-active': activeIndex === index, 'is-hovered': hoveredCard === index }
                ]"
              >
                ...
                <div class="image-wrapper">
                  ...
                  <div class="quick-actions">
                    <a
                      :href="cert.imageUrl || 'javascript:void(0)'"
                      target="_blank"
                    ...
                    <a
                      :href="cert.documentUrl || cert.driveUrl || 'javascript:void(0)'"
                      target="_blank"
                      rel="noopener noreferrer"
                      class="action-btn"
                      title="Buka Dokumen"
                ...
                <div class="card-body">
                  <div class="card-category">{{ cert.categoryLabel[currentLang] }}</div>
                  ...
                  <a :href="cert.documentUrl || cert.driveUrl || 'javascript:void(0)'" target="_blank" rel="noopener noreferrer" class="primary-button">
                    <component :is="Eye" class="w-4 h-4" />
        ...
        >
          <div :class="['certificate-card-grid', { 'is-hovered': hoveredCard === index }]">
            <div class="grid-card-inner">
              <div class="grid-card-front">
                ...
                <div class="grid-overlay">
                  <div class="grid-category-badge">{{ cert.categoryLabel[currentLang] }}</div>
            ...
            <div class="grid-info">
              ...
              <a :href="cert.documentUrl || cert.driveUrl || 'javascript:void(0)'" target="_blank" rel="noopener noreferrer" class="grid-link" title="Buka Dokumen">
                <component :is="ExternalLink" class="w-4 h-4" />
              </a>
            </div>
          </div>
...
const Cloud = createIcon('<path d="M17.5 19H9a7 7 0 1 1 6.71-9h1.79a4.5 4.5 0 1 1 0 9Z"/>')
const Scale = createIcon(
  '<path d="m16 16 3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1Z"/><path d="m2 16 3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1Z"/><path d="M7 21h10"/><path d="M12 3v18"/><path d="M3 7h2c2 0 5-1 7-2 2 1 5 2 7 2h2"/>'
)
const Languages = createIcon(
...
const categories = [
  ...
  {
    id: 'programming',
    label: { id: 'Pemrograman', en: 'Programming' },
    icon: Code
  },
  {
    id: 'backend',
  ...
  {
    id: 'cloud-ai',
    label: { id: 'Cloud & AI', en: 'Cloud & AI' },
    icon: Cloud
  },
  {
    id: 'best-practice',
    label: { id: 'Best Practice', en: 'Best Practice' },
    icon: Target
  },
  {
    id: 'management',
...
// Computed: Stats Data (Otomatis menghitung dari database)
const statsData = computed(() => {
  const totalCertificates = certificates.value ? certificates.value.length : 0

  let uniqueSkillsCount = 0
  if (certificates.value) {
    const allSkills = certificates.value.flatMap((cert) => cert.skills || [])
  ...
  return [
    ...
    },
    {
      icon: Trophy,
      value: '100%',
      label: { id: 'Completion Rate', en: 'Completion Rate' },
      color: 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)'
    },
    {
      icon: Target,
      value: `${uniqueSkillsCount}+`,
      label: { id: 'Teknologi', en: 'Technologies' },
      color: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)'
    }
  ]
})

// Certificates Data dengan kategori
const { data: certificates, refresh } = await useFetch('/api/certificates')

// Computed: Filtered Certificates
const filteredCertificates = computed(() => {
  // PENGAMAN: Jika data dari Supabase kosong/gagal, kembalikan array kosong
  if (!certificates.value) return []

  if (selectedCategory.value === 'all') {
    return certificates.value
  }
  return certificates.value.filter((cert) => cert.categoryId === selectedCategory.value)
})
...
const getCategoryCount = (categoryId) => {
  // PENGAMAN: Jika data kosong, kembalikan angka 0
  if (!certificates.value) return 0

  if (categoryId === 'all') {
    return certificates.value.length
  }
  return certificates.value.filter((cert) => cert.categoryId === categoryId).length
}
...
const nextSlide = () => {
  if (activeIndex.value < filteredCertificates.value.length - 1) {
    activeIndex.value++
  }
}
...
// Watch category changes to reset carousel index
